$.ajax({
    url: "http://10.30.210.115:8081/users/getImageList",
    type: 'get',
    dataType:'json',
    success:function(data){
        if(data.statusCode === 0 ){
            ...
            for(let item of imgList){
                let img_type = item.img_type;
                let oLi;
                //大图
                if(img_type === 0){
                    oLi = $('<li><img src="../images/upload/meilifengguang/'+item.img_src+'" alt=""></li>');
                    $('.slider .slider-img-ul').append(oLi);
                }
            }
            (function($){
